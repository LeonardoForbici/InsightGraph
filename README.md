...
Codebase
   ↓
⚡ Fast Scan (Scout Layer)
   ↓
📦 Extração de entidades (classes, funções, módulos)
   ↓
🔎 Indexação semântica (RAG)
   ↓
🧠 Análise profunda (Deep Layer)
   ↓
🕸️ Persistência no Neo4j
   ↓
📊 Visualização + Query inteligente
🧩 Principais Features
🔍 Análise de Código
