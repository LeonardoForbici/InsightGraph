...
interface WatchModePanelProps {
  ...
  onStartWatch: (path: string) => Promise<void>;
  onStopWatch: () => Promise<void>;
  onClearHistory: () => void;
  onViewImpact?: (impact: ImpactResult) => void;
}

export default function WatchModePanel({
  connected,
  watching,
  watchedPath,
  lastImpact,
  ...
  const handleStart = async () => {
    const targetPath = projectPath.trim() || watchedPath?.trim();
    if (!targetPath) {
    ...
    <div style={{
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <p style={{ margin: '8px 0 0 0', color: '#94a3b8', fontSize: '0.85rem' }}>
          Monitore mudancas em tempo real e acione atualizacao automatica do grafo
        </p>
      </div>
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: '0.75rem', color: '#94a3b8', marginBottom: '4px' }}>Conexao</div>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: '0.9rem', fontWeight: '600', color: watching ? '#60a5fa' : '#94a3b8' }}>
            {watching ? 'Monitorando' : 'Parado'}
          </div>
        </div>

        <div style={{ flex: 1 }}>
          <div style={{ fontSize: '0.75rem', color: '#94a3b8', marginBottom: '4px' }}>Impactos</div>
      ...
      {!watching ? (
        <div style={{ marginBottom: '20px' }}>
          ...
          <input
            ...
            placeholder={watchedPath || 'C:\\caminho\\do\\projeto'}
            disabled={loading}
            style={{
              width: '100%',
              padding: '10px 12px',
      ...
      {watchedPath && (
        <div style={{ marginBottom: 14, color: '#94a3b8', fontSize: 12 }}>
          Path ativo: {watchedPath}
        </div>
      )}
      ...
      {lastImpact && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: '0.75rem', color: '#94a3b8' }}>Ultimo Impacto</div>
            <div style={{ fontSize: '0.75rem', color: '#94a3b8' }}>{formatTimestamp(lastImpact.timestamp)}</div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '12px' }}>
            <div>
              <div style={{ fontSize: '0.7rem', color: '#94a3b8', marginBottom: '4px' }}>Alterados</div>
              <div style={{ fontSize: '1.1rem', fontWeight: '600', color: '#60a5fa' }}>{lastImpact.changed_nodes.length}</div>
            </div>
            <div>
              <div style={{ fontSize: '0.7rem', color: '#94a3b8', marginBottom: '4px' }}>Impactados</div>
              <div style={{ fontSize: '1.1rem', fontWeight: '600', color: '#f59e0b' }}>{lastImpact.affected_nodes.length}</div>
            </div>
            <div>
              <div style={{ fontSize: '0.7rem', color: '#94a3b8', marginBottom: '4px' }}>Risco</div>
              <div style={{ fontSize: '1.1rem', fontWeight: '600', color: getRiskColor(lastImpact.risk_score) }}>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <div style={{ fontSize: '0.85rem', color: '#cbd5e1', fontWeight: '600' }}>
              Historico ({impactHistory.length})
            </div>
            <button
            ...
            {impactHistory.map((impact, idx) => (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                  <span style={{ color: '#e2e8f0', fontWeight: '600' }}>{impact.file.split('/').pop()}</span>
                  <span style={{ color: '#94a3b8' }}>{formatTimestamp(impact.timestamp)}</span>
                </div>
